...
- **profile persona**：profile 可配置 `persona`（绝对路径 Markdown 文件），controller 启动 pi 时通过 `--append-system-prompt` 注入，用于按 profile 挂载身份/情景提示词（如 `prompts/persona-remote-qq.md`）；不配置则不注入。
