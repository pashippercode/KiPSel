# Pi Agent Environment (local workstation)
...
- As of 2026-08-08 the `gpt-5.6-sol`/`gpt-5.6-terra` channels are quota-limited (upstream 402 via a zen proxy, resets ~22:33 CST); use `zen-mimo-v2.5` for image tasks meanwhile.
...
- Mobile app connects to the bridge over the tailnet with the mobile token.
